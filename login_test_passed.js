const webdriver = require('selenium-webdriver'),
 ...
 driver.get("http://seleniumdemo.com/").then( async function() {
    ...
    // let logout = By.xpath("/div/section/div/div/nav/ul/li[6]/a")
    // let logout2 = await common.waitForElementVisible(logout);
    // logout2.click()
    ...
    let emailLog2 = await common.waitForElementVisible(emailLog);
    emailLog2.sendKeys("test@test.com")

    let passwordLog = By.id("password");
    let passwordLog2 = await common.waitForElementVisible(passwordLog);
    passwordLog2.sendKeys("test@test.com")

    let login = By.name("login");
    let login2 = await common.waitForElementVisible(login);
    login2.click()
    ...
    const text = (await driver
        .findElement(By.xpath('//*[@id="page-7"]/div/section/div/div/nav/ul/li[6]/a'))
        .getText());

      if (text && text.indexOf(" Logout ") <= 0) {
